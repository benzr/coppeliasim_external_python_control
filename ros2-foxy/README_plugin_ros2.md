...
pip install catkin_pkg empy lark  pyparsing pyyaml setuptools argcomplete xmlschema
```

Changes in  meta/interfaces.txt
remove : example_interfaces/action/Fibonacci (compilation error)
add : geometry_msgs/msg/Twist

```
colcon build --symlink-install --cmake-args ' -DCMAKE_CXX_FLAGS=-fpermissive'
```